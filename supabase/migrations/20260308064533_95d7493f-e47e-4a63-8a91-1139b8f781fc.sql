...
-- Fix issue_invoice to snapshot template when issuing
...
BEGIN
  ...
  -- Snapshot template if set
  IF _invoice.template_id IS NOT NULL THEN
    SELECT jsonb_build_object(
      'name', t.name,
      'layout', t.layout,
      'styles', t.styles,
      'watermark_required', COALESCE(t.watermark_required, true),
      'supports_branding', COALESCE(t.supports_branding, true)
    ) INTO _tpl_snapshot
    FROM invoice_templates t
    WHERE t.id = _invoice.template_id;
  END IF;
  ...
  UPDATE invoices SET
    ...
    template_snapshot = COALESCE(_tpl_snapshot, _invoice.template_snapshot)
