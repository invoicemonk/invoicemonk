...
BEGIN
  ...
  -- Snapshot payment method if linked
  ...
  -- Generate immutable values
  v_verification_id := gen_random_uuid();
  v_issued_at := now();

  -- Build hash input (deterministic string)
  v_hash_input := concat(
    v_invoice.id::text, '|',
  ...
  -- Update invoice with all immutable fields
  UPDATE invoices SET
    status = 'sent',
  ...
  -- Lock currency on business if first invoice
