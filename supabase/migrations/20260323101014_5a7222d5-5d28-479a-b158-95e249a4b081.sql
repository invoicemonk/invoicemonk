-- Fix: Replace invalid 'overdue' enum value with valid 'sent' in both overloads

-- Overload 1: 2 params
CREATE OR REPLACE FUNCTION public.get_receivables_intelligence(_business_id uuid, _currency_account_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  FROM (
    SELECT 
      ...
      COALESCE(SUM(
        CASE WHEN i.status IN ('issued', 'sent', 'viewed') AND i.total_amount > i.amount_paid 
...
-- Overload 2: 4 params
CREATE OR REPLACE FUNCTION public.get_receivables_intelligence(_business_id uuid, _currency_account_id uuid DEFAULT NULL::uuid, _start_date date DEFAULT NULL::date, _end_date date DEFAULT NULL::date)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
