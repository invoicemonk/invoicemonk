...
BEGIN
  ...
  IF v_client.email IS NULL OR trim(v_client.email) = '' THEN
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      ...
      'message', 'Client email address is missing'
    ));
    v_score := v_score - 5;
    v_has_warn := true;
  ELSE
  ...
  -- 2. missing_client_name (warn, -5)
  IF v_client.name IS NULL OR trim(v_client.name) = '' THEN
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_client_name', 'rule_type', 'field_presence',
      'severity', 'warn', 'passed', false,
  ...
  ELSE
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_client_name', 'rule_type', 'field_presence',
      'severity', 'warn', 'passed', true,
      'message', 'Client name present'
    ));
  END IF;

  -- 3. missing_tax_schema (warn, -10)
  IF v_business.is_vat_registered = true AND v_invoice.tax_schema_id IS NULL THEN
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_tax_schema', 'rule_type', 'tax_compliance',
      'severity', 'warn', 'passed', false,
      'message', 'VAT-registered business but no tax schema applied'
    ));
    v_score := v_score - 10;
    v_has_warn := true;
  ELSE
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_tax_schema', 'rule_type', 'tax_compliance',
      'severity', 'warn', 'passed', true,
      'message', 'Tax schema check passed'
    ));
  END IF;

  -- 4. amount_mismatch (block, -30)
  ...
  IF abs(v_line_item_sum - v_invoice.subtotal) > 0.01 THEN
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'amount_mismatch', 'rule_type', 'calculation',
  ...
  ELSE
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      ...
      'message', 'Amounts match correctly'
    ));
  END IF;

  -- 5. missing_due_date (warn, -5)
  IF v_invoice.due_date IS NULL THEN
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_due_date', 'rule_type', 'field_presence',
      'severity', 'warn', 'passed', false,
      'message', 'Invoice has no due date set'
    ));
    v_score := v_score - 5;
    v_has_warn := true;
  ELSE
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_due_date', 'rule_type', 'field_presence',
      'severity', 'warn', 'passed', true,
      'message', 'Due date is set'
    ));
  END IF;

  -- 6. missing_business_tax_id (warn, -10)
  IF v_business.tax_id IS NULL OR trim(v_business.tax_id) = '' THEN
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_business_tax_id', 'rule_type', 'field_presence',
      'severity', 'warn', 'passed', false,
      'message', 'Business has no tax ID configured'
    ));
    v_score := v_score - 10;
    v_has_warn := true;
  ELSE
    v_checks := v_checks || jsonb_build_array(jsonb_build_object(
      'rule_key', 'missing_business_tax_id', 'rule_type', 'field_presence',
      'severity', 'warn', 'passed', true,
      'message', 'Business tax ID present'
    ));
  END IF;
  ...
  v_compliance_hash := encode(extensions.digest(v_compliance_result::text::bytea, 'sha256'), 'hex');
