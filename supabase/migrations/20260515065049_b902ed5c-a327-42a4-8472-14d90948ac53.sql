...
BEGIN
  ...
  SELECT * INTO v_sensitive FROM public.business_sensitive_data WHERE business_id = v_invoice.business_id;
  v_tax_id := v_sensitive.tax_id;
  v_vat_reg := v_sensitive.vat_registration_number;
  v_gov_id_type := v_sensitive.government_id_type;
  v_gov_id_value := v_sensitive.government_id_value;
  ...
  IF v_business.entity_type != 'individual' THEN
    ...
    END IF;
    IF (v_tax_id IS NULL OR trim(v_tax_id) = '')
       AND (v_gov_id_value IS NULL OR trim(v_gov_id_value) = '') THEN
  ...
  SET status = 'issued',
      ...
      issuer_snapshot = jsonb_build_object(
        ...
        'government_id_type', v_gov_id_type,
        'government_id_value', v_gov_id_value
      ),
