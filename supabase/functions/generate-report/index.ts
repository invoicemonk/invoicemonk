...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // PDF format — generate styled HTML report
    if (format === 'pdf') {
      // Get business name for the report header
      let businessName = 'Business';
      if (businessId) {
        const { data: biz } = await supabase.from('businesses').select('name, legal_name').eq('id', businessId).maybeSingle();
        if (biz) businessName = biz.legal_name || biz.name;
      }
      ...
      if (dataArray.length > 0) {
        ...
        tableHeaders = keys.map(k => `<th style="border:1px solid #ddd;padding:8px 12px;background:#f8f9fa;text-align:left;font-size:12px;">${k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</th>`).join('');
        tableRows = dataArray.map(row => {
          const r = row as Record<string, unknown>;
          return '<tr>' + keys.map(k => {
            const val = r[k];
            const display = val === null || val === undefined ? '' : typeof val === 'object' ? JSON.stringify(val) : String(val);
            return `<td style="border:1px solid #ddd;padding:6px 12px;font-size:12px;">${display}</td>`;
          }).join('') + '</tr>';
        }).join('');
      }
      ...
      const summaryHtml = Object.entries(summary).map(([k, v]) =>
        `<tr><td style="padding:4px 12px;font-weight:600;font-size:13px;">${k.replace(/_/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</td><td style="padding:4px 12px;font-size:13px;">${typeof v === 'object' ? JSON.stringify(v) : v}</td></tr>`
      ).join('');

      const html = `<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>${report_type.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())} — ${businessName}</title>
<style>body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;margin:40px;color:#333}
h1{font-size:22px;margin-bottom:4px}h2{font-size:16px;color:#666;margin-top:24px;margin-bottom:8px}
...
<body>
<h1>${report_type.replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase())}</h1>
<div class="meta">${businessName} · ${reportCurrency} · Period: ${year} · Generated: ${generatedAt}</div>
<h2>Summary</h2>
<table>${summaryHtml}</table>
<h2>Details (${dataArray.length} records)</h2>
<table><thead><tr>${tableHeaders}</tr></thead><tbody>${tableRows}</tbody></table>
<div class="footer">
<p>This report was generated by Invoicemonk on ${generatedAt}.</p>
<p>Report type: ${report_type} · Currency: ${reportCurrency} · Year: ${year}</p>
</div>
</body></html>`;
      ...
      return new Response(html, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="${report_type}-${reportCurrency}-${year}.html"`,
        },
      });
    }
